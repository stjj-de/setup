...
sudo sed -i -r 's/(listen .*443)/\1;#/g; s/(ssl_(certificate|certificate_key|trusted_certificate) )/#;#\1/g' /etc/nginx/sites-enabled/stjj.de.conf /etc/nginx/sites-enabled/analytics.stjj.de.conf
...
sudo sed -i -r 's/#?;#//g' /etc/nginx/sites-enabled/stjj.de.conf /etc/nginx/sites-enabled/analytics.stjj.de.conf
